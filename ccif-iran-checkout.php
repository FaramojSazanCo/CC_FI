<?php
/**
 * Plugin Name: CCIF Iran Checkout (Rebuild for Analysis)
...
class CCIF_Iran_Checkout_For_Analysis {

    public function __construct() {
        // This hook is to define the fields, but not render them yet.
        add_filter( 'woocommerce_checkout_fields', [ $this, 'define_custom_fields' ] );

        // This hook will be used to render the fields in our custom layout
        add_action( 'woocommerce_before_checkout_billing_form', [ $this, 'render_custom_billing_form' ] );

        // This hook removes the default rendering of the billing form
    ...
    public function define_custom_fields( $fields ) {
        ...
        // First, clear out the default billing fields.
        $fields['billing'] = [];
        ...
        $fields['billing']['billing_national_code'] = ['label' => 'کد ملی', 'class' => ['form-row-wide', 'ccif-person-field', 'ccif-real-person-field'], 'placeholder' => '۱۰ رقم بدون خط تیره'];
        ...
        $fields['billing']['billing_postcode'] = ['label' => 'کد پستی', 'required' => true, 'type' => 'tel', 'class' => ['form-row-first', 'ccif-address-field']];
        $fields['billing']['billing_phone'] = ['label' => 'شماره تماس', 'required' => true, 'type' => 'tel', 'class' => ['form-row-last', 'ccif-address-field']];
    ...
    public function enqueue_assets() {
        ...
        wp_enqueue_script( 'ccif-checkout-js', plugin_dir_url( __FILE__ ) . 'assets/js/ccif-checkout.js', ['jquery'], '7.1', true );
        wp_localize_script( 'ccif-checkout-js', 'ccifData', [ 'cities' => $this->load_iran_data()['cities'] ] );
        wp_enqueue_style( 'ccif-checkout-css', plugin_dir_url( __FILE__ ) . 'assets/css/ccif-checkout.css', [], '7.1' );
    }
}
new CCIF_Iran_Checkout_For_Analysis();
